                    ...
                    <div class="card-body">
                        ...
                        <div class="row mg-t-20">
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>تاريخ الفاتوره</span> <span>{{ $singleInvoice->invoice_date }}</span></p>
                                <p class="invoice-info-row"><span>الدكتور</span> <span></span>{{ $singleInvoice->Doctor->name }}</p>
                                <p class="invoice-info-row"><span>القسم</span> <span></span>{{ $singleInvoice->Section->name }}</p>
                            </div>
                        </div>
                        <div class="table-responsive mg-t-40">
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                <tr>
                                    <td>1</td>
                                    <td class="tx-12">{{ $singleInvoice->Service->name }}</td>
                                    <td class="tx-center">{{ $singleInvoice->price }}</td>
                                    <td class="tx-right">{{$singleInvoice->type == 1 ? 'نقدي' : 'اجل'}}</td>
                                </tr>
                                <tr>
                                    ...
                                    <td class="tx-right" colspan="2"> {{number_format($singleInvoice->price, 2)}}</td>
                                </tr>
                                <tr>
                                    <td class="tx-right">قيمة الخصم</td>
                                    <td class="tx-right" colspan="2">{{$singleInvoice->discount_value}}</td>
                                </tr>
                                <tr>
                                    <td class="tx-right">نسبة الضريبة</td>
                                    <td class="tx-right" colspan="2">% {{$singleInvoice->tax_rate}}</td>
                                </tr>
                                <tr>
                                    <td class="tx-right tx-uppercase tx-bold tx-inverse">الاجمالي شامل الضريبه</td>
                                    <td class="tx-right" colspan="2">
                                        <h4 class="tx-primary tx-bold">{{number_format($singleInvoice->total_with_tax, 2)}}</h4>
