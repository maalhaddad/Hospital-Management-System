<div>
    ...
    <form wire:submit.prevent="SaveGroup" autocomplete="off">
        <div class="form-group">
            ...
            <input type="text"
            wire:model="name_group"
             name="name_group"
             class="form-control @error('name_group') is-invalid @enderror "
              >
        ...
        <div class="card mt-4">
            <div class="card-header">
                <div class="col-md-12">
                    <button wire:click.prevent="addService" class="btn btn-outline-primary">اضافة خدمة فرعية</button>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                @else
                                    <input type="number" name="GroupsItems[{{$index}}][quantity]"
                                           class="form-control" wire:model="GroupsItems.{{$index}}.quantity" value="{{ $groupItem['quantity'] ?? '' }}"/>
                                @endif
                                </td>
                                <td>
                                    @if ($groupItem['is_saved'])
                                    <button wire:click.prevent="editService({{ $index }})" class="btn btn-sm btn-primary">تعديل</button>
                                    @else
                                    <button wire:click.prevent="saveService({{ $index }})" class="btn btn-sm btn-success mr-1">تاكيد</button>

                                    @endif

                                    <button wire:click.prevent ="RemovrService({{ $index }})" class="btn btn-sm btn-danger">حذف</button>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
